...
/**
 * <p>
 * This {@code LoginModule} authenticates clients by validating their SAML
 * assertions with an external security token service (such as PicketLinkSTS).
 * If the supplied assertion contains roles, these roles are extracted and
 * included in the {@code Group} returned by the {@code getRoleSets} method.
 * </p>
 * <p>
 * This module defines the following module options:
 * <ul>
 * <li>
 * configFile - this property identifies the properties file that will be used
 * to establish communication with the external security token service.</li>
 * <li>
 * cache.invalidation: set it to true if you require invalidation of JBoss Auth
 * Cache at SAML Principal expiration.</li>
 * <li>
 * jboss.security.security_domain: name of the security domain where this login
 * module is configured. This is only required if the cache.invalidation option
 * is configured.</li>
 * <li>
 * roleKey: a comma separated list of strings that define the attributes in SAML
 * assertion for user roles</li>
 * <li>
 * localValidation: if you want to validate the assertion locally for signature
 * and expiry</li>
 * <li>
 * localValidationSecurityDomain: the security domain for the trust store
 * information (via the JaasSecurityDomain)</li>
 * <li>
 * tokenEncodingType: encoding type of SAML token delivered via http request's
 * header. Possible values are: base64 - content encoded as base64. In case of
 * encoding will vary between base64 and gzip use base64 and LoginModule will
 * detect gzipped data. gzip - gzipped content encoded as base64 none - content
 * not encoded in any way</li>
 * <li>
 * samlTokenHttpHeader - name of http request header to fetch SAML token from.
 * For example: "Authorize"</li>
 * <li>
 * samlTokenHttpHeaderRegEx - Java regular expression to be used to get SAML
 * token from "samlTokenHttpHeader". Example: use: ."(.)".* to parse SAML token
 * from header content like this: SAML_assertion="HHDHS=", at the same time set
 * samlTokenHttpHeaderRegExGroup to 1.</li>
 * <li>
 * samlTokenHttpHeaderRegExGroup - Group value to be used when parsing out value
 * of http request header specified by "samlTokenHttpHeader" using
 * "samlTokenHttpHeaderRegEx".</li>
 * </ul>
 * </p>
 * <p>
 * Any properties specified besides the above properties are assumed to be used
 * to configure how the {@code STSClient} will connect to the STS. For example,
 * the JBossWS {@code StubExt.PROPERTY_SOCKET_FACTORY} can be specified in order
 * to inform the socket factory that must be used to connect to the STS. All
 * properties will be set in the request context of the {@code Dispatch}
 * instance used by the {@code STSClient} to send requests to the STS.
 * </p>
 * <p>
 ...
 * </pre>
 * 
 * The first three properties specify the STS endpoint URL, service name, and
 * port name. The last two properties specify the username and password that are
 * to be used by the application server to authenticate to the STS and have the
 * SAML assertions validated.
 * </p>
 * <p>
 * <b>NOTE:</b> Sub-classes can use {@link #getSTSClient()} method to customize
 * the {@link STSClient} class to make calls to STS/
 * </p>
 * 
...
public abstract class CUDSAML2STSCommonLoginModule extends
	...
	/**
	 * Options that are computed by this login module. Few options are removed
	 * and the rest are set in the dispatch sts call
	 */
	...
	/**
	 * This is an option that should identify the configuration file for
	 * WSTrustClient.
	 */
	 ...
	 * 
	 * @see
	 * org.jboss.security.auth.spi.AbstractServerLoginModule#initialize(javax
	...
	public void initialize(Subject subject, CallbackHandler callbackHandler,
		...
		// save the config file and cache validation options, removing them from
		// the map - all remaining properties will
		// be set in the request context of the Dispatch instance used to send
		// requests to the STS.
		this.stsConfigurationFile = (String) this.options
				.remove(STS_CONFIG_FILE);
		String cacheInvalidation = (String) this.options
				.remove("cache.invalidation");
		if (cacheInvalidation != null && !cacheInvalidation.isEmpty()) {
			this.enableCacheInvalidation = Boolean
					.parseBoolean(cacheInvalidation);

			this.securityDomain = (String) this.options
					.remove(SecurityConstants.SECURITY_DOMAIN_OPTION);
			if (this.securityDomain == null || this.securityDomain.isEmpty())
				throw logger
						.optionNotSet(SecurityConstants.SECURITY_DOMAIN_OPTION);
		}
		...
		if (StringUtil.isNotNull(localValidationStr)) {
			...
			localValidationSecurityDomain = (String) options
					.get("localValidationSecurityDomain");
			...
			String localTestingOnlyStr = (String) options
					.get("localTestingOnly");
	...
	public boolean login() throws LoginException {
		...
		try {
			if (getSamlTokenHttpHeader() != null) {
				...
				if (callback.getCredential() instanceof String) {
					callback.setCredential(new SamlCredential(DocumentUtil
							.getDocument(callback.getCredential().toString())
							.getDocumentElement()));
				}
		...
		if (localValidation) {
			...
			try {
				...
				// if the STS says the assertion is invalid, throw an exception
				// to signal that authentication has failed.
				if (isValid == false)
		...
		try {
			...
			if (subject != null) {
				...
				if (baseID instanceof NameIDType) {
					...
					// If the user has configured cache invalidation of subject
					// based on saml token expiry
					if (enableCacheInvalidation) {
						TimeCacheExpiry cacheExpiry = this.getCacheExpiry();
						XMLGregorianCalendar expiry = AssertionUtil
								.getExpiration(assertion);
						if (expiry != null) {
							Date expiryDate = expiry.toGregorianCalendar()
									.getTime();

							logger.trace("Creating Cache Entry for JBoss at ["
									+ new Date()
									+ "] , with expiration set to SAML expiry = "
									+ expiryDate);

							cacheExpiry.register(securityDomain, expiryDate,
									principal);
						} else {
							logger.samlAssertionWithoutExpiration(assertion
									.getID());
						}
					}
				}
		...
		if (getUseFirstPass()) {
			super.sharedState.put("javax.security.auth.login.name",
					this.principal);
	...
	public boolean commit() throws LoginException {
		if (super.commit()) {
			final boolean added = subject.getPublicCredentials().add(
					this.credential);
	...
	protected Group[] getRoleSets() throws LoginException {
		if (this.assertion == null) {
			try {
				this.assertion = SAMLUtil.fromElement(this.credential
		...
		if (logger.isTraceEnabled()) {
			try {
				logger.trace("Assertion from where roles will be sought = "
	...
	protected STSClient getSTSClient() {
		/*
		 * Builder builder = new Builder(this.stsConfigurationFile); STSClient
		...
		if (rawOptions.containsKey(STS_CONFIG_FILE)) {
			...
			builder.portName((String) rawOptions.get(PORT_NAME)).serviceName(
					(String) rawOptions.get(SERVICE_NAME));
			builder.username((String) rawOptions.get(USERNAME_KEY)).password(
					(String) rawOptions.get(PASSWORD_KEY));

			String passwordString = (String) rawOptions.get(PASSWORD_KEY);
			if (passwordString != null
					&& passwordString
							.startsWith(PicketLinkFederationConstants.PASS_MASK_PREFIX)) {
				// password is masked
				String salt = (String) rawOptions
						.get(PicketLinkFederationConstants.SALT);
				if (StringUtil.isNullOrEmpty(salt))
					throw logger.optionNotSet("Salt");

				String iCount = (String) rawOptions
						.get(PicketLinkFederationConstants.ITERATION_COUNT);
		...
		// if the login module options map still contains any properties, assume
		// they are for configuring the connection
		...
		if (!this.options.isEmpty()) {
			...
			for (Map.Entry<String, ?> entry : this.options.entrySet())
				dispatch.getRequestContext().put(entry.getKey(),
						entry.getValue());
		}
		return client;
	}

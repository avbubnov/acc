...
public class IHUCCert extends IHUCCertBase implements IHLocal {
	...
	// proc_uccert_info_file=System.getProperty("jboss.server.config.url")+"proc_uccert_info.properties";
	...
	public BaseParamItem process_start(BaseParamItem paramMap) throws Exception {
		 ...
		 * 
		 * int batch=0; while(trans<currentTime){ batch++;
		 * trans+=period*24*60*60*1000; //trans+=period*1000; if(batch % 100 ==
		 * 0){ log.info("IHUCCert:process_start:batch:"+batch); } }
			...
			public void run() {
				...
				try {
					...
					try {
						...
						log.error("IHUCCert:process_start:run:finally:error:"
								+ e);
					}
				}
			}
	...
	public BaseParamItem process_stop(BaseParamItem paramMap) throws Exception {
		...
		try {
			...
			// Contexts.getApplicationContext().get("UCCertScheduled");
	...
	private synchronized void process_start_content() throws Exception {
		...
		try {
			...
			file_reestr = content();
			// file_reestr = new
			// File("/Проекты/ЦУД разработка 5.0/Сертификаты/"+file_name);
			...
			System.out.println("IHUCCert:process_start_content:03:"
					+ persist_record);
	...
	public File content() {
		...
		System.out.println("IHUCCert:content:path:" + path);
		...
		try {
			...
			try {
				...
				System.out.println("LaunchCRLTask:content:finally:is:error:"
						+ e);
			}
		}
			...
			if (s != null) {
				...
				while ((s = br.readLine()) != null) {
					...
					if (s.startsWith("|")) {
						...
						if ((i_logich % 100) == 0) {
							System.out.println("file_interaction:03:"
									+ i_logich);

						}
						if (persist_record != 0 && (persist_record % 100) == 0) {
							System.out.println("file_interaction:04:commit+:"
									+ persist_record);
	...
	public void records_db_wh(PreparedStatement ps, String rec)
		...
		try {
			...
			for (String pss : sa) {
				 ...
				 * 
				 * if(i==1){ sbn.append(sconfrec);
				 * sbv.append("'"+(pss.equals("")
				 * ?"":pss.trim().replaceAll("'","''"))+"'"); }else{
				 * 
				 * sbn.append(", "+sconfrec);
				 * sbv.append(", '"+(pss.equals("")?""
				 * :pss.trim().replaceAll("'","''"))+"'");
			...
			/*
			 * query="INSERT into UC_CERT_REESTR (ID_SRV, "+sbn+
			...
			// System.out.println("records_db_wh:error:rec:"+rec);

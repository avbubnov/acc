...
@Stateless
//@LocalBinding(jndiBinding=ServiceReestr.ArchiveAuditFunc)
...
public class IHArchiveAuditFunc extends IHArchiveBase implements IHLocal {
  ...
  // private static String file_path="/distr/jboss/data/audit/func/";
   private static String file_path="/home/jboss/jboss/data/audit/func/";
  ...
  // private static final String proc_aafunc_info_file=System.getProperty("jboss.server.config.url")+"proc_aafunc_info.properties";
   ...
   public BaseParamItem process_start(BaseParamItem paramMap)throws Exception{
	...
	ScheduledFuture shf =  scheduler.scheduleAtFixedRate(new Runnable() {
		  ...
		  //    }
				...
				try {
					...
					log.error("IHArchiveAuditFunc:process_start:run:finally:error:"+e);
				}
			 }    
	 }
	...
	if(TaskProcessor.getControls().containsKey("archiveAuditFuncScheduled")){
		try{
			TaskProcessor.getControls().get("archiveAuditFuncScheduled").cancel(false);
		}catch(Exception e){
	...
	log.info("IHArchiveAuditFunc:stopTask:01");
	 ...
	 try{
    	// ScheduledFuture shf = (ScheduledFuture) Contexts.getApplicationContext().get("archiveAuditSysScheduled");
    	 ...
    	 ScheduledFuture shf =TaskProcessor.getControls().get("archiveAuditFuncScheduled");
   ...
   private synchronized void process_start_content(Long archiveParamValue) throws Exception{
	    ...
	    try{
	    	 ...
	    	 List<String> los = em.createNativeQuery(
	    	  ...
	    	  log.info("IHArchiveAuditFunc:process_start_content:monthInterval:"+monthInterval);
	    	  
	    	/* List<Object[]> lo = em.createNativeQuery(
                                  "select to_char(AL.CREATED , 'YYYY_MM') vdate, AL.ID_SRV, AL.UP_ACTIONS, AL.UP_TOKEN, "+
                                  ...
                                  "where AL.CREATED < to_date('01.'||to_char(SYSDATE - INTERVAL '"+monthInterval+"' month, 'MM.YYYY'),'DD.MM.YYYY') "+ 
                                  "order by AL.CREATED ")
	    	  ...
	    	  for(Object[] objectArray :lo){
	    		  if(prev_date==null||!prev_date.equals(objectArray[0].toString())){
	    			  ...
	    			  file=new File(file_path+"audit_func_"+objectArray[0].toString()+".txt");
	    	...
	    	//  jpi.put("list", JiList) ;
			 ...
			 try{
				 ...
				 log.info("IHArchiveAuditFunc:process_start_content:finally:hit:"+hit);
						 
					   DateFormat df = new SimpleDateFormat ("dd.MM.yy HH:mm:ss");
					  // URL url = new URL(proc_aafunc_info_file);
				...
				log.error("IHArchiveAuditFunc:process_start_content:error:2:"+e);
			 }finally{
				 try {
					if(os!=null){
						os.close();
   ...
   private static Long calendar() {
		// System.out.println("calendar:01");
		 ...
		 System.out.println("calendar:03:start:"+start);

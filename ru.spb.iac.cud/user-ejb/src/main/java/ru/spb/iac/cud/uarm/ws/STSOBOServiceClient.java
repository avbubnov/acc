	...
	*/
     ...
     */
		...
		try{
		   ...
		   //cxf2.7    
           //  System/.setProperty/("org.apache.cxf.stax.allowInsecurePar/ser", /"true");
         
			/*
			//test-contur
			Hostn/ameVerifier hv = new Ho/stnameVerifier() {
				pu/blic b/oolean/ verify(Str/ing urlHostN/ame, SSLS/ession session) {
								 logg/er.info("Warning: URL Host: "+ urlHostName+ " vs. " + session.getPeerHost());
								 retur/n true;
				  }
			};
			HttpsURLConne/ction.setDe/faultHostnameVerifier(hv); 
           */
		 ...
		 if(publicKey==null) {	
			...
			//Ser/vice jaxwsSe/rvice = Ser/vice.crea/te(servi/ce);
			//jaxwsSer/vic/e.addPort(portNam/e, soapBi/nding, end/pointURI);
		
			//не нужен :
			//либо - Service/.create(wsdlLocation, servi/ce) и нет ad/dPort
			//либо - Servic/e.create(service) и add/Port
			//jaxwsSer/vice.addPor/t(portName2,/ soapBinding, endpoi/ntURI);
		  ...
		  /*
		    //test-contur
		    HTTPCondui/t httpConduit = (HTTPCo/nduit)  (/(org.apache.cxf.jaxws.DispatchImpl)dispatch).getClient().getConduit() ; 
			//HTTPCon/duit httpConduit = (HTTPCo/nduit) ClientProxy.getClient(proxy).getConduit();
           	TLSClient/Parameters tlsCP = new TLSClientP/arameters();
			final SSLS/ocketFactoryImpl sslFact = new S/SLSocketFactoryImpl();
			tlsCP.set/SSLSocketFactory(sslFac/t);
			tlsCP.se/tDisableCNCheck(true);
			httpCondu/it.setTlsClientParamet/ers(tlsCP);
			*/
			...
			transformList.add(transformC14N);
			 ...
			 try{
			     ...
			     // req/uest.setUseKey(useKeyType);
   		       //   requ/est.setKeyType(uri);

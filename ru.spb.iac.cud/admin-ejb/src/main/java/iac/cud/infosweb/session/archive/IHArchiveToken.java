...
@Stateless
//@LocalBinding(jndiBinding=ServiceReestr.ArchiveToken)
 ...
 //  private static final String proc_atoken_info_file=System.getProperty("jboss.server.config.url")+"proc_atoken_info.properties";
   ...
   public BaseParamItem process_start(BaseParamItem paramMap)throws Exception{
	...
	ScheduledFuture shf =  scheduler.scheduleAtFixedRate(new Runnable() {
		  ...
		  //    }
				...
				try {
					...
					log.error("IHArchiveToken:process_start:run:finally:error:"+e);
				}
			 }    
	 }
	...
	if(TaskProcessor.getControls().containsKey("archiveTokenScheduled")){
		try{
			TaskProcessor.getControls().get("archiveTokenScheduled").cancel(false);
		}catch(Exception e){
	...
	log.info("IHConfLoadData:stopTask:01");
	 ...
	 try{
    	// ScheduledFuture shf = (ScheduledFuture) Contexts.getApplicationContext().get("archiveAuditSysScheduled");
    	 ...
    	 ScheduledFuture shf =TaskProcessor.getControls().get("archiveTokenScheduled");
   ...
   private synchronized  void process_start_content() throws Exception{
	    ...
	    try{
	    	 ...
	    	 List<String> los = em.createNativeQuery(
	    	  ...
	    	  log.info("IHArchiveToken:process_start_content:monthInterval:"+monthInterval);
	    	  ...
	    	  List<Object[]> lo = em.createNativeQuery(
                                  ...
                                  "from TOKEN_KNL_T tt "+
                                  "where tt.CREATED < to_date('01.'||to_char(SYSDATE - INTERVAL '"+monthInterval+"' month, 'MM.YYYY'),'DD.MM.YYYY')  "+
                                  "order by tt.CREATED ")
	    	  ...
	    	  for(Object[] objectArray :lo){
	    		  if(prev_date==null||!prev_date.equals(objectArray[0].toString())){
	    			  ...
	    			  file=new File(file_path+"audit_token_"+objectArray[0].toString()+".txt");
	    	...
	    	//  jpi.put("list", JiList) ;
			 ...
			 try{
				 ...
				 log.info("IHArchiveToken:process_start_content:finally:hit:"+hit);
